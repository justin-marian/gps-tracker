#!/usr/bin/env python3
"""
NMEA GPS Emulator for SI Tema 2.

Adapted from https://github.com/luk-kop/nmea-gps-emulator (MIT licensed).
...
# Path to the FIFO device of the emulated usb-serial
...
NAV_DATA = {
    ...
    'position': {
        'latitude_value': '4419.000',
        'latitude_direction': 'N',
        'longitude_value': '02636.000',
...
class NmeaMsg:
    ...
    def __init__(self, position: dict, altitude: float, speed: float, heading: float):
        # Instance attributes
        print("Initialize the UTC Date Time NOW")
        self.utc_date_time = datetime.datetime.utcnow()#datetime.datetime.now(datetime.UTC)
    ...
    def position_update(self, utc_date_time_prev: datetime):
        ...
        """
        print("Update LAT/LONG positions")
...
def gps_simulator(ser):
    print("START GPS SIMULATOR")
    try:

        # Initialize NmeaMsg object
        nmea_obj = NmeaMsg(position=NAV_DATA['position'],
                           altitude=NAV_DATA['gps_altitude_amsl'],
                           speed=NAV_DATA['gps_speed'],
                           heading=NAV_DATA['gps_heading'])
        while True:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        serial_fifo = sys.argv[1]
    if serial_fifo == "-":
        gps_simulator(sys.stdout.buffer)
    else:
        with open(serial_fifo, "wb") as ser:
            gps_simulator(ser)
